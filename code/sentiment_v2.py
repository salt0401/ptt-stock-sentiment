"""Improved sentiment analysis module (v2).

Key improvements over sentiment.py:
1. Skip-gram W2V (sg=1) with higher vector_size=150, min_count=5
2. No PCA dimensionality reduction — uses raw W2V vectors
3. Cosine distance k-NN propagation (k=20) instead of all-words Minkowski IDW
...
# ---------------------------------------------------------------------------
# Word2Vec v2 — skip-gram, higher dim, min_count filtering
# ---------------------------------------------------------------------------

def train_word2vec_v2(sentences, vector_size=150, window=5, min_count=5,
...
def map_sentiment_scores_v2(vocab, opinion_dict, word_freq=None):
...
def split_scored_unscored_v2(merged_df, vectors):
...
def build_sentiment_dictionary_v2(model, opinion_dict, word_freq=None,
    ...
    df_unscored, df_scored, vecs_unscored, vecs_scored = \
        split_scored_unscored_v2(mapped_df, vectors)
